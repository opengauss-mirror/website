$(document).ready(function () {
  ...
  var privateMethods = {
    ...
    jumpTpDetail: function () {
      $(".content-box .content-title,.content-box .content-desc").on(
        "click",
        function () {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (data.status === 201) {
            ...
            $(".detail-title").append(
              '<p class="active" key="">' +
                privateMethods.transformLang("all") +
                "<span> (0) </span></p>"
            );
            ...
            data.obj.totalNum.forEach(function (item) {
              ...
              if (item.key === curTab) {
                if (item.key === "all") {
                  $(".detail-title").append(
                    '<p class="active" key="">' +
                      privateMethods.transformLang(item.key) +
                      "<span> (" +
                      item.count +
                      ") </span>" +
                      "</p>"
          ...
          data.obj.records.forEach(function (item, index) {
            var contentPath = privateMethods.getBlogUrl(
              item.articleName,
              item.type
            );
            let a =
              '<div class="content-box">' +
              '<p class="content-title" path="' +
              contentPath +
              '" type="' +
              item.type +
              '">' +
              item.title +
              '</p><p class="content-desc" path="' +
              contentPath +
              '" type="' +
              item.type +
              '">' +
              ...
              "来自" +
              '：<span class="tag">' +
              privateMethods.transformLang(item.type) +
              " " +
              "" +
              "</span></p></div>";
            ...
            if (lang === "zh") {
              if (item.type === "docs") {
                $(".detail-content").append(
                  `<div class="content-box" data-index="${index + 1}">` +
                    '<p class="content-title" path="' +
                    contentPath +
                    '" type="' +
                    item.type +
              ...
              if (item.type === "docs") {
                $(".detail-content").append(
                  `<div class="content-box" data-weather="${index + 1}">` +
                    '<p class="content-title" path="' +
                    contentPath +
                    '" type="' +
                    item.type +
                ...
                $(".detail-content").append(
                  `<div class="content-box" data-weather="${index + 1}">` +
                    ...
                    '">' +
                    item.title +
                    '</p><p className="content-desc" path="' +
                    contentPath +
                    '" type="' +
                    item.type +
                    '">' +
  ...
  var init = function () {
    $(".search-input").val(
      decodeURI(privateMethods.getUrlParams("keyword")) || ""
    );
    remoteMethods.getSearchData(
      {
        keyword: $(".search-input").val() || "",
