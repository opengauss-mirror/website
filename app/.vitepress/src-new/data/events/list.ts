...
export const listData = {
  zh: zh
    .map((item, id) => ({ ...item, pic: cityPicMap.get(item.location || '') || defaultPic, id }))
    .sort((a, b) => {
      const replacePattern = /\//g;
      const dateA = new Date(a.date.replace(replacePattern, '-'));
      const dateB = new Date(b.date.replace(replacePattern, '-'));
